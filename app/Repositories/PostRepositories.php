...
class PostRepositories implements PostInterfaces {
    ...
    public function store($request) {
        ...
        $page->created_by = isset($request['created_by']) ? $request['created_by'] : 0;
        $page->updated_by = isset($request['updated_by']) ? $request['updated_by'] : 0;
    ...
    public function update($id, $request) {
        ...
        if ($page->save()) {
            if (isset($request['type']) && ($request['type'] == 'custom_page' || $request['type'] == 'department_details' || $request['type'] == 'programs_details')) {
                ...
                if (!is_null($trans)) {
                    $trans->update(['title' => $request['title'], 'content' => $content, 'short_content' => $request['short_content']]);
                } else {
                    $page->translations()->create(['title' => $request['title'], 'content' => $content, 'short_content' => $request['short_content'], 'lang' => $request['lang']]);
                }
            }
            return 'success';
        }
