...
<table class="table table-bordered table-striped table-hover dataTable">
    <thead>
            <tr class="text-center">
                ...
                <th>Address</th>
                <th>Date</th>
                @if(dsld_check_permission(['edit-vandors']))
                <th>Status</th>
                @endif
                @if(dsld_check_permission(['edit-vandors','delete-vandors']))
                <th>Action</th>
                @endif
            </tr>
        </thead>
        ...
        @if(is_array($data) || count($data) > 0 )
            ...
            @php $key++; @endphp
                <tr @if($value->user_type == 'admin') style="background:#ffe9e9;" @endif>
                    ...
                    <td>
                        <a href="{{ route('vendor.edit', [$value->id]) }}">{{ $value->name }}</a><br>
                    </td>
                    ...
                    <td>
                        ...
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="{{$value->id }}" onchange="DSLDStatusUpdate('{{ $value->id }}','{{ $value->banned == 0 ? 1 : 0  }}', '{{ route('vendor.status') }}','{{ csrf_token() }}')" @if($value->banned == 0) checked @endif >
                    ...
                    <td>
                        <p class="text-center mb-0 action_items">
                            ...
                            @if(dsld_check_permission(['edit-vandors']))
                            <a href="{{ route('vendor.edit', [$value->id]) }}" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                    <i class="zmdi zmdi-edit"></i>
                                </a>
                            @endif
                            @endif
                            @if(dsld_check_permission(['delete-vandors']))
